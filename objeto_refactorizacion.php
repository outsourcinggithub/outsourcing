 ...
 */
    ...
    class objeto_refactorizacionPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('objeto_refactorizacionssearch', $this->dataset,
                array('id', 'id_paquete', 'nombre', 'ruta', 'tipo', 'aplicacion', 'nuevo', 'id_noconforme'),
                array($this->RenderText('Id'), $this->RenderText('CMO'), $this->RenderText('Nombre del Objeto'), $this->RenderText('Ruta'), $this->RenderText('Tipo'), $this->RenderText('Aplicación'), $this->RenderText('Nuevo'), $this->RenderText('Número NC')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('objeto_refactorizacionasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_noconforme', $this->RenderText('Número NC')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('objeto_refactorizacionGrid_ruta_handler_list');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for id_noconforme field
            //
            $column = new TextViewColumn('id_noconforme', 'Número NC', $this->dataset);
            $column->SetOrderable(true);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('objeto_refactorizacionGrid_ruta_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for id_noconforme field
            //
            $editor = new TextEdit('id_noconforme_edit');
            $editor->SetMaxLength(64);
            $editColumn = new CustomEditColumn('Número NC', 'id_noconforme', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'objeto_refactorizacion_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'objeto_refactorizacionGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('ruta', 'Ruta', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'objeto_refactorizacionGrid_ruta_handler_list', $column);
